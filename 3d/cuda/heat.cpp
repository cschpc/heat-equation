...
/*    MPI_Type_create_subarray(3, sizes, subsizes, offsets, MPI_ORDER_C,
    ...
    GPU_CHECK( cudaMalloc(&parallel.send_buffers[1][1], (nx + 2) * (nz + 2) * sizeof(double)) );
    GPU_CHECK( cudaMalloc(&parallel.send_buffers[2][0], (nx + 2) * (ny + 2) * sizeof(double)) );
    GPU_CHECK( cudaMalloc(&parallel.send_buffers[2][1], (nx + 2) * (ny + 2) * sizeof(double)) );
    GPU_CHECK( cudaMalloc(&parallel.recv_buffers[0][0], (ny + 2) * (nz + 2) * sizeof(double)) );
    GPU_CHECK( cudaMalloc(&parallel.recv_buffers[0][1], (ny + 2) * (nz + 2) * sizeof(double)) );
    GPU_CHECK( cudaMalloc(&parallel.recv_buffers[1][0], (nx + 2) * (nz + 2) * sizeof(double)) );
