...
program heat_solve
  ...
  if (parallelization % rank == 0) then
     write(*,*) 'Average temperature at start: ', average_temp
  end if

  ! Largest stable time step
  dt = current%inner_field%dx**2 * current%inner_field%dy**2 * current%inner_field%dz**2 / &
       & (2.0 * a * (current%inner_field%dx**2 + current%inner_field%dy**2 + current%inner_field%dz**2))
  ...
  do iter = 1, nsteps
     ...
     ! senrecv is blocking, so no need of barriers and so on
     call exchange(previous, parallelization)

     call evolve(current%inner_field, previous%inner_field, a, dt)
